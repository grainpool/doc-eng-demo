...
function prBody(input: PublishInput, committed: PublishPatch[], refused: string[]): string {
  const lines: string[] = [
    ...
    "",
    "> ⚠️ **This run's fact change was a Change Lab HYPOTHETICAL applied to a working copy —",
    "> deployed product truth is unchanged.** Merging this PR would make the docs assert the",
    "> hypothetical value. Review the mechanics, then close; the cleanup cron closes stale run",
    "> PRs after 48 hours. Merge only if the underlying product change has actually shipped.",
    "",
    "## Fact deltas",
    "",
