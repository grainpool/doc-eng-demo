 ...
 * FROM eval-report.json. The page is a deliverable, not an apology
 * (validation.md §6 honesty rules): actual misses, actual false positives,
 * and why each happens, linked to the doc unit.
 */
...
const report = JSON.parse(readFileSync(join(root, "eval-report.json"), "utf8")) as {
  generated_at: string;
  corpus_size: number;
  metrics: Record<string, unknown> & {
    detection_recall_per_class: Record<string, { recall: number; detected: number; total: number }>;
  };
...
`;
writeFileSync(join(root, "packages", "concord-web", "public", "failures.html"), html);
